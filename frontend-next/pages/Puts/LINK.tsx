...
class LINK extends React.Component {
    ...
    getStrikes = async () =>{
        ...
        if (ethereum) {
          ...
          await factoryContract.getAntiOptionStrikePrices(token2Address).then((result:any ) =>{
            strikePriceOptions.push(ethers.utils.formatEther(result).slice(0,6))
          })
    
          this.setState({strikePriceOption:strikePriceOptions});

          await factoryContract.getAntiOptionStrikeDeadline(token2Address).then((result:any ) =>{
    ...
    getBalanceOptions = async () => {
      const { ethereum } = window;
        if (ethereum) {
          ...
          const factoryContract = new ethers.Contract(factoryAddress, factoryABI, signer);
          await factoryContract.getAmountAntiOptions(token2Address,accounts[0]).then((result:any ) =>{
    ...
    buyOption = async () => {
      ...
      if (ethereum) {
        ...
        await factoryContract.buyAntiOptions(token2Address,ethers.utils.parseEther(this.state.amountOptions),{ value: payFee })
      }else{
        console.log("Ethereum object does not exist");
      }
    }
    sellOption = async () => {
      ...
      if (ethereum) {
        ...
        const factoryContract = new ethers.Contract(factoryAddress, factoryABI, signer);
        await factoryContract.exerciseAntiOption(token2Address,this.state.amountOptions)
      }else{
        console.log("Ethereum object does not exist");
      }
    }
